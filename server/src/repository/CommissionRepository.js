...
export default {
  ...
  */
   ...
   */
    ...
    // })
  // get agg com that trx.transactionType === POS
  async getUserPOSCommissionHistory(userId: string, query?: {
    skip?: number,
    limit?: number,
    date?: string,
    startDate?: string,
    ...
    const data = await CommissionHistory
      ...
      .limit(paginate.limit)
      .sort({'meta.createdAt': -1})
      .populate({ path: 'transaction', model: 'Transaction',  match: { transactionType: enumType.transactionType.POS }})
      .populate('user', 'firstName lastName _id email')
      .populate('wallet', '-monifyAccountReference -monifyReservetionReference')
      .populate('commissionWallet, transaction');
